...
import { phoneGlb, envMap } from "../assets";
...
loader.load(phoneGlb, function (gltf) {
  //gltf加载成功后返回一个对象
  ...
  mesh.material = new THREE.MeshStandardMaterial({
    ...
    map: texLoader.load(envMap.basecolor), //颜色贴图
    // 金属度、粗糙度贴图表示的值会和金属度、粗糙度分别相乘
    roughnessMap: texLoader.load(envMap.roughness), //粗糙度贴图
    metalnessMap: texLoader.load(envMap.metallic), //金属度贴图
    // 相机镜头等位置需要设置半透明效果(设置alphaMap和transparent属性)
    alphaMap: texLoader.load(envMap.opacity), //alpha贴图
    normalMap: texLoader.load(envMap.normal), //法线贴图
