...
JavaScript 的对象可用于描述现实世界中的某个对象。例如，为了描述 Mr.Duan，我们可以用若干键值对来描述他:
...
上述对象申明了一个 `name` 属性，值是 `'Mr.Duan'`，`birth` 属性，值是 `1998`，以及其他一些属性。最后，把这个对象赋值给变量 `mrhope` 后，就可以通过变量 `mrhope` 来获取 Mr.Duan 的属性了:
