<template><div><p>建立硬链接和软链接非常简单，那这两种链接有什么区别? 它们都有什么作用? 这才是链接文件最不容易理解的地方，我们分别来讲讲。</p>
<!-- more -->
<h2 id="ln-创建硬链接" tabindex="-1"><a class="header-anchor" href="#ln-创建硬链接"><span>ln 创建硬链接</span></a></h2>
...
<figure><img src="@source/linux/assets/hardlink.jpg" alt="Hardlink" tabindex="0" loading="lazy"><figcaption>Hardlink</figcaption></figure>
...
<p>当我们查找一个文件，比如 <code v-pre>/root/test</code> 时，要经过以下步骤:</p>
<ul>
<li>首先找到根目录的 inode(根目录的 inode 是系统已知的，inode 号是 2)，然后判断用户是否有权限访问根目录的 block。</li>
<li>如果有权限，则可以在根目录的 block 中访问到 <code v-pre>/root</code> 的文件名及对应的 inode 号。</li>
<li>通过 <code v-pre>/root/</code> 目录的 inode 号，可以查找到 <code v-pre>/root/</code> 目录的 inode 信息，接着判断用户是否有权限访问 <code v-pre>/root/</code> 目录的 block。</li>
<li>如果有权限，则可以从 <code v-pre>/root/</code> 目录的 block 中读取到 test 文件的文件名及对应的 inode 号。</li>
<li>通过 test 文件的 inode 号，就可以找到 test 文件的 inode 信息，接着判断用户是否有权限访问 test 文件的 block。</li>
<li>如果有权限，则可以读取 block 中的数据，这样就完成了 <code v-pre>/root/test</code> 文件的读取与访问。</li>
</ul>
<p>按照这个步骤，在给源文件 <code v-pre>/root/test</code> 建立了硬链接文件 <code v-pre>/tmp/test-hard</code> 之后，在 <code v-pre>/root/</code> 目录和 <code v-pre>/tmp/</code> 目录的 block 中就会建立 test 和 test-hard 的信息，这个信息主要就是文件名和对应的 inode 号。但是我们会发现 test 和 test-hard 的 inode 信息居然是一样的，那么，我们无论访问哪个文件，最终都会访问 inode 号是 262147 的文件信息。</p>
...
<h2 id="ln-创建软链接" tabindex="-1"><a class="header-anchor" href="#ln-创建软链接"><span>ln 创建软链接</span></a></h2>
...
<p>软链接主要是为了照顾管理员的使用习惯。比如，有些系统的自启动文件 <code v-pre>/etc/rc.local</code> 放置在 <code v-pre>/etc</code> 目录中，而有些系统却将其放置在 <code v-pre>/etc/rc.d/rc.local</code> 中，那么干脆对这两个文件建立软链接，不论您习惯操作哪一个文件，结果都是一样的。</p>
<p>如果您比较细心，则应该已经发现软链接和源文件的 inode 号是不一致的，我们也画一张示意图来看看软链接的原理，如图 3 所示。</p>
<figure><img src="@source/linux/assets/softlink.jpg" alt="软链接示意图" tabindex="0" loading="lazy"><figcaption>软链接示意图</figcaption></figure>
...
<li>如果有权限访问根目录的 block，就会在 block 中查找到 <code v-pre>/tmp/</code> 目录的 inode 号。</li>
<li>接着访问 <code v-pre>/tmp/</code> 目录的 inode 信息，判断用户是否有权限访问 <code v-pre>/tmp/</code> 目录的 block。</li>
