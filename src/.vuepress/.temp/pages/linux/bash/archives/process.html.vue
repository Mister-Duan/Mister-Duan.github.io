...
<h2 id="ps" tabindex="-1"><a class="header-anchor" href="#ps"><span>ps</span></a></h2>
<p><code v-pre>ps</code> 命令用来列出进程信息。</p>
...
</div><p>不带任何参数时，<code v-pre>ps</code> 只列出与当前 Session 相关的进程。输出结果中，<code v-pre>PID</code> 是进程 ID、<code v-pre>TTY</code> 是进程的终端号(如果显示<code v-pre>?</code>，则表示进程没有终端)，<code v-pre>TIME</code> 是消耗的 CPU 时间，<code v-pre>CMD</code> 是触发进程的命令。</p>
<p><code v-pre>x</code> 参数列出所有进程的详细信息，包括不在当前 Session 的信息。</p>
...
</div><p>这时的输出结果，会多出 <code v-pre>STAT</code> 一栏，表示状态。它的各种值如下。</p>
<ul>
<li><code v-pre>R</code> 正在运行或准备运行</li>
<li><code v-pre>S</code> 正在睡眠，即没有运行，正在等待一个事件唤醒</li>
<li><code v-pre>D</code> 不可中断睡眠。进程正在等待 I/O，比如磁盘驱动器的 I/O</li>
<li><code v-pre>T</code> 已停止，即进程停止运行</li>
<li><code v-pre>Z</code> “僵尸”进程。即这是一个已经终止的子进程，但父进程还没有清空它(没有把子进程从进程表中删除)</li>
<li><code v-pre>&lt;</code> 高优先级进程。这可能会授予一个进程更多重要的资源，给它更多的 CPU 时间。</li>
<li><code v-pre>N</code> 低优先级进程。一个低优先级进程(一个“好”进程)只有当其它高优先级进程执行之后，才会得到处理器时间。</li>
</ul>
<p><code v-pre>aux</code> 参数可以显示更多信息。</p>
...
<ul>
<li><code v-pre>USER</code> 用户 ID，表示进程的所有者</li>
<li><code v-pre>%CPU</code> 百分比表示的 CPU 使用率</li>
<li><code v-pre>%MEM</code> 百分比表示的内存使用率</li>
<li><code v-pre>VSZ</code> 虚拟内存大小</li>
<li><code v-pre>RSS</code> 进程占用的物理内存的大小，以千字节为单位。</li>
<li><code v-pre>START</code> 进程运行的起始时间。若超过 24 小时，则用天表示。</li>
</ul>
<h2 id="top" tabindex="-1"><a class="header-anchor" href="#top"><span>top</span></a></h2>
<p><code v-pre>top</code> 命令可以查看机器的当前状态。</p>
...
<p><code v-pre>jobs</code> 命令用来查看后台任务。</p>
...
</div><p>输出结果之中，每个后台任务会有一个编号。上面结果中，<code v-pre>xlogo</code> 的编号是 <code v-pre>1</code>，<code v-pre>+</code> 表示正在运行。</p>
<h2 id="fg" tabindex="-1"><a class="header-anchor" href="#fg"><span>fg</span></a></h2>
<p><code v-pre>fg</code> 命令用于将后台任务切换到前台。</p>
...
</div><p><code v-pre>fg</code> 命令之后，跟随着一个百分号和工作序号，用来指定切换哪一个后台任务。如果只有一个后台任务，那么 <code v-pre>fg</code> 命令可以不带参数。</p>
<h2 id="bg" tabindex="-1"><a class="header-anchor" href="#bg"><span>bg</span></a></h2>
<p><code v-pre>bg</code> 命令用于将一个暂停的前台任务，转移到后台。只有暂停的任务，才能使用 <code v-pre>bg</code> 命令，因为正在运行的任务，命令行是无法输入的。</p>
...
</div><p><code v-pre>Ctrl + z</code> 可以暂停正在运行的前台任务。</p>
<h2 id="kill" tabindex="-1"><a class="header-anchor" href="#kill"><span>kill</span></a></h2>
<p><code v-pre>kill</code> 命令用于杀死进程。它的参数是进程 ID。</p>
...
</div><p><code v-pre>kill</code> 命令的实质是操作系统向进程发送信号。在使用 Ctrl-c 的情况下，会发送一个叫做 INT(中断)的信号；当使用 Ctrl-z 时，则发送一个叫做 TSTP(终端停止)的信号。</p>
<p><code v-pre>kill</code> 命令可以用来向进程发送指定信号。</p>
...
<li>INT: 编号 2，中断。实现和 <code v-pre>Ctrl-c</code> 一样的功能，由终端发送。通常，它会终止一个程序。</li>
...
<p><code v-pre>-l</code> 参数可以列出所有信号。</p>
...
</div><h2 id="killall" tabindex="-1"><a class="header-anchor" href="#killall"><span>killall</span></a></h2>
<p><code v-pre>killall</code> 命令用于向指定的程序或用户发送信号。</p>
...
</div><h2 id="其他进程相关命令" tabindex="-1"><a class="header-anchor" href="#其他进程相关命令"><span>其他进程相关命令</span></a></h2>
<ul>
<li><code v-pre>pstree</code> 输出树型结构的进程列表，这个列表展示了进程间父/子关系。</li>
<li><code v-pre>vmstat</code> 输出一个系统资源使用快照，包括内存，交换分区和磁盘 I/O。为了看到连续的显示结果，则在命令名后加上延时的时间(以秒为单位)。例如，“vmstat 5”。终止输出，按下 Ctrl-c 组合键。</li>
<li><code v-pre>xload</code> 一个图形界面程序，可以画出系统负载的图形。</li>
<li><code v-pre>tload</code> 与 <code v-pre>xload</code> 程序相似，但是在终端中画出图形。使用 Ctrl-c，来终止输出。</li>
</ul>
</div></template>
