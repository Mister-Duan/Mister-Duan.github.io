<template><div><p>Bash 是 Unix 系统和 Linux 系统的一种 Shell(命令行环境)，是目前绝大多数 Linux 发行版的默认 Shell。</p>
<!-- more -->
<h2 id="shell-的含义" tabindex="-1"><a class="header-anchor" href="#shell-的含义"><span>Shell 的含义</span></a></h2>
...
<h2 id="shell-的种类" tabindex="-1"><a class="header-anchor" href="#shell-的种类"><span>Shell 的种类</span></a></h2>
...
</div><p>上面两个命令中，<code v-pre>$</code> 是命令行环境的提示符，用户只需要输入提示符后面的内容。</p>
<p>Linux 允许每个用户使用不同的 Shell，用户的默认 Shell 一般都是 Bash，或者与 Bash 兼容。</p>
<h2 id="命令行环境" tabindex="-1"><a class="header-anchor" href="#命令行环境"><span>命令行环境</span></a></h2>
<h3 id="终端模拟器" tabindex="-1"><a class="header-anchor" href="#终端模拟器"><span>终端模拟器</span></a></h3>
...
<h3 id="命令行提示符" tabindex="-1"><a class="header-anchor" href="#命令行提示符"><span>命令行提示符</span></a></h3>
<p>进入命令行环境以后，用户会看到 Shell 的提示符。提示符往往是一串前缀，最后以一个美元符号 <code v-pre>$</code> 结尾，用户可以在这个符号后面输入各种命令。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#383A42;--shiki-dark:#ABB2BF">[user@hostname] $</span></span></code></pre>
</div><p>上面例子中，完整的提示符是 <code v-pre>[user@hostname] $</code>，其中前缀是用户名(<code v-pre>user</code>)加上 <code v-pre>@</code>，再加主机名(<code v-pre>hostname</code>)。比如，用户名是 <code v-pre>bill</code>，主机名是 <code v-pre>home-machine</code>，前缀就是 <code v-pre>bill@home-machine</code>。</p>
...
<p>为了简洁，后文的命令行提示符都只使用 <code v-pre>$</code> 表示。</p>
<h3 id="进入和退出方法" tabindex="-1"><a class="header-anchor" href="#进入和退出方法"><span>进入和退出方法</span></a></h3>
<p>进入命令行环境以后，一般就已经打开 Bash 了。如果您的 Shell 不是 Bash，可以输入 <code v-pre>bash</code> 命令启动 Bash。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">bash</span></span></code></pre>
</div><p>退出 Bash 环境，可以使用 <code v-pre>exit</code> 命令，也可以同时按下 <code v-pre>Ctrl + d</code>。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#0184BC;--shiki-dark:#56B6C2">exit</span></span></code></pre>
</div><p>Bash 的基本用法就是在命令行输入各种命令，非常直观。作为练习，可以试着输入 <code v-pre>pwd</code> 命令。按下回车键，就会显示当前所在的目录。</p>
...
</div><p>如果不小心输入了 <code v-pre>pwe</code>，会返回一个提示，表示输入出错，没有对应的可执行程序。</p>
...
</div><h2 id="shell-和-bash-的历史" tabindex="-1"><a class="header-anchor" href="#shell-和-bash-的历史"><span>Shell 和 Bash 的历史</span></a></h2>
<p>Shell 伴随着 Unix 系统的诞生而诞生。</p>
<p>1969 年，Ken Thompson 和 Dennis Ritchie 开发了第一版的 Unix。</p>
<p>1971 年，Ken Thompson 编写了最初的 Shell，称为 Thompson shell，程序名是 <code v-pre>sh</code>，方便用户使用 Unix。</p>
...
<p>1978 年，加州大学伯克利分校的 Bill Joy 开发了 C shell，为 Shell 提供 C 语言的语法，程序名是 <code v-pre>csh</code>。它是第一个真正替代 <code v-pre>sh</code> 的 UNIX shell，被合并到 Berkeley UNIX 的 2BSD 版本中。</p>
<p>1979 年，UNIX 第七版发布，内置了 Bourne Shell，导致它成为 Unix 的默认 Shell。注意，Thompson shell、Mashey shell 和 Bourne shell 都是贝尔实验室的产品，程序名都是 <code v-pre>sh</code>。对于用户来说，它们是同一个东西，只是底层代码不同而已。</p>
<p>1983 年，David Korn 开发了 Korn shell，程序名是 <code v-pre>ksh</code>。</p>
<p>1985 年，Richard Stallman 成立了自由软件基金会(FSF)，由于 Shell 的版权属于贝尔公司，所以他决定写一个自由版权的、使用 GNU 许可证的 Shell 程序，避免 Unix 的版权争议。</p>
<p>1988 年，自由软件基金会的第一个付薪程序员 Brian Fox 写了一个 Shell，功能基本上是 Bourne shell 的克隆，叫做 Bourne-Again SHell，简称 Bash，程序名为 <code v-pre>bash</code>，任何人都可以免费使用。后来，它逐渐成为 Linux 系统的标准 Shell。</p>
<p>1989 年，Bash 发布 1.0 版。</p>
<p>1996 年，Bash 发布 2.0 版。</p>
<p>2004 年，Bash 发布 3.0 版。</p>
<p>2009 年，Bash 发布 4.0 版。</p>
<p>2019 年，Bash 发布 5.0 版。</p>
<p>用户可以通过 <code v-pre>bash</code> 命令的 <code v-pre>--version</code> 参数或者环境变量 <code v-pre>$BASH_VERSION</code>，查看本机的 Bash 版本。</p>
