<template><div><p>Bash 脚本有时需要创建临时文件或临时目录。常见的做法是，在 <code v-pre>/tmp</code> 目录里面创建文件或目录，这样做有很多弊端，使用 <code v-pre>mktemp</code> 命令是最安全的做法。</p>
<!-- more -->
<h2 id="临时文件的安全问题" tabindex="-1"><a class="header-anchor" href="#临时文件的安全问题"><span>临时文件的安全问题</span></a></h2>
<p>直接创建临时文件，尤其在 <code v-pre>/tmp</code> 目录里面，往往会导致安全问题。</p>
<p>首先，<code v-pre>/tmp</code> 目录是所有人可读写的，任何用户都可以往该目录里面写文件。创建的临时文件也是所有人可读的。</p>
...
</div><p>上面命令在 <code v-pre>/tmp</code> 目录直接创建文件，该文件默认是所有人可读的。</p>
...
<li>脚本退出时，要删除临时文件(使用 <code v-pre>trap</code> 命令)。</li>
</ul>
</blockquote>
<h2 id="mktemp-命令的用法" tabindex="-1"><a class="header-anchor" href="#mktemp-命令的用法"><span>mktemp 命令的用法</span></a></h2>
<p><code v-pre>mktemp</code> 命令就是为安全创建临时文件而设计的。虽然在创建临时文件之前，它不会检查临时文件是否存在，但是它支持唯一文件名和清除机制，因此可以减轻安全攻击的风险。</p>
<p>直接运行 <code v-pre>mktemp</code> 命令，就能生成一个临时文件。</p>
...
</div><p>上面命令中，<code v-pre>mktemp</code> 命令生成的临时文件名是随机的，而且权限是只有用户本人可读写。</p>
<p>Bash 脚本使用 <code v-pre>mktemp</code> 命令的用法如下。</p>
...
<span class="line"><span style="--shiki-light:#0184BC;--shiki-dark:#56B6C2">echo</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> "Our temp file is </span><span style="--shiki-light:#E45649;--shiki-dark:#E06C75">$TMPFILE</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379">"</span></span></code></pre>
</div><p>为了确保临时文件创建成功，<code v-pre>mktemp</code> 命令后面最好使用 OR 运算符 (<code v-pre>||</code>)，保证创建失败时退出脚本。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#A0A1A7;--shiki-light-font-style:italic;--shiki-dark:#7F848E;--shiki-dark-font-style:italic">#!/bin/bash</span></span>
<span class="line"></span>
...
<span class="line"><span style="--shiki-light:#0184BC;--shiki-dark:#56B6C2">echo</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> "Our temp file is </span><span style="--shiki-light:#E45649;--shiki-dark:#E06C75">$TMPFILE</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379">"</span></span></code></pre>
</div><p>为了保证脚本退出时临时文件被删除，可以使用 <code v-pre>trap</code> 命令指定退出时的清除操作。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#A0A1A7;--shiki-light-font-style:italic;--shiki-dark:#7F848E;--shiki-dark-font-style:italic">#!/bin/bash</span></span>
<span class="line"></span>
...
<span class="line"><span style="--shiki-light:#E45649;--shiki-dark:#E06C75">TMPFILE</span><span style="--shiki-light:#383A42;--shiki-dark:#56B6C2">=</span><span style="--shiki-light:#383A42;--shiki-dark:#ABB2BF">$(</span><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">mktemp</span><span style="--shiki-light:#383A42;--shiki-dark:#ABB2BF">) || </span><span style="--shiki-light:#0184BC;--shiki-dark:#56B6C2">exit</span><span style="--shiki-light:#986801;--shiki-dark:#D19A66"> 1</span></span>
<span class="line"><span style="--shiki-light:#0184BC;--shiki-dark:#56B6C2">echo</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> "Our temp file is </span><span style="--shiki-light:#E45649;--shiki-dark:#E06C75">$TMPFILE</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379">"</span></span></code></pre>
</div><h2 id="mktemp-命令的参数" tabindex="-1"><a class="header-anchor" href="#mktemp-命令的参数"><span>mktemp 命令的参数</span></a></h2>
<p><code v-pre>-d</code> 参数可以创建一个临时目录。</p>
...
<span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">/tmp/tmp.Wcau5UjmN6</span></span></code></pre>
</div><p><code v-pre>-p</code> 参数可以指定临时文件所在的目录。默认是使用 <code v-pre>$TMPDIR</code> 环境变量指定的目录，如果这个变量没设置，那么使用 <code v-pre>/tmp</code> 目录。</p>
...
<span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">/home/ruanyf/tmp.FOKEtvs2H3</span></span></code></pre>
</div><p><code v-pre>-t</code> 参数可以指定临时文件的文件名模板，模板的末尾必须至少包含三个连续的 <code v-pre>X</code> 字符，表示随机字符，建议至少使用六个 <code v-pre>X</code>。默认的文件名模板是 <code v-pre>tmp.</code> 后接十个随机字符。</p>
...
<span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">/tmp/mytemp.yZ1HgZV</span></span></code></pre>
</div><h2 id="trap-命令" tabindex="-1"><a class="header-anchor" href="#trap-命令"><span>trap 命令</span></a></h2>
<p><code v-pre>trap</code> 命令用来在 Bash 脚本中响应系统信号。</p>
<p>最常见的系统信号就是 SIGINT(中断)，即按 <code v-pre>Ctrl + C</code> 所产生的信号。<code v-pre>trap</code> 命令的 <code v-pre>-l</code> 参数，可以列出所有的系统信号。</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>trap</code> 的命令格式如下。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#0184BC;--shiki-dark:#56B6C2">trap</span><span style="--shiki-light:#383A42;--shiki-dark:#ABB2BF"> [动作] [信号1] [信号2] ...</span></span></code></pre>
</div><p>上面代码中，“动作”是一个 Bash 命令，“信号”常用的有以下几个。</p>
<blockquote>
<ul>
...
<li>TERM: 编号 15，这是 <code v-pre>kill</code> 命令发出的默认信号。</li>
<li>EXIT: 编号 0，这不是系统信号，而是 Bash 脚本特有的信号，不管什么情况，只要退出脚本就会产生。</li>
</ul>
</blockquote>
<p><code v-pre>trap</code> 命令响应 <code v-pre>EXIT</code> 信号的写法如下。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#0184BC;--shiki-dark:#56B6C2">trap</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> 'rm -f "$TMPFILE"'</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> EXIT</span></span></code></pre>
</div><p>上面命令中，脚本遇到 <code v-pre>EXIT</code> 信号时，就会执行 <code v-pre>rm -f &quot;$TMPFILE&quot;</code>。</p>
...
</div><p>上面代码中，不管是脚本正常执行结束，还是用户按 <code v-pre>Ctrl + C</code> 终止，都会产生 <code v-pre>EXIT</code> 信号，从而触发删除临时文件。</p>
<p>注意，<code v-pre>trap</code> 命令必须放在脚本的开头。否则，它上方的任何命令导致脚本退出，都不会被它捕获。</p>
<p>如果 <code v-pre>trap</code> 需要触发多条命令，可以封装一个 Bash 函数。</p>
...
<span class="line"></span>
<span class="line"><span style="--shiki-light:#0184BC;--shiki-dark:#56B6C2">trap</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> egress</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> EXIT</span></span></code></pre>
