...
<p><code v-pre>globbing</code> 这个词，来自于早期的 Unix 系统有一个 <code v-pre>/etc/glob</code> 文件，保存扩展的模板。后来 Bash 内置了这个功能，但是这个名字就保留了下来。</p>
...
</div><h2 id="波浪线扩展" tabindex="-1"><a class="header-anchor" href="#波浪线扩展"><span>波浪线扩展</span></a></h2>
<p>波浪线 <code v-pre>~</code> 会自动扩展成当前用户的主目录。</p>
...
</div><p><code v-pre>~/dir</code> 表示扩展成主目录的某个子目录，<code v-pre>dir</code> 是主目录里面的一个子目录名。</p>
...
</div><p><code v-pre>~user</code> 表示扩展成用户 <code v-pre>user</code> 的主目录。</p>
...
<p>如果 <code v-pre>~user</code> 的 <code v-pre>user</code> 是不存在的用户名，则波浪号扩展不起作用。</p>
...
</div><p><code v-pre>~+</code> 会扩展成当前所在的目录，等同于 <code v-pre>pwd</code> 命令。</p>
...
</div><h2 id="字符扩展" tabindex="-1"><a class="header-anchor" href="#字符扩展"><span><code v-pre>?</code> 字符扩展</span></a></h2>
...
</div><p>上面命令中，<code v-pre>?</code> 表示单个字符，所以会同时匹配 <code v-pre>a.txt</code> 和 <code v-pre>b.txt</code>。</p>
...
</div><p>上面命令中，<code v-pre>??</code> 匹配了两个字符。</p>
<p><code v-pre>?</code> 字符扩展属于文件名扩展，只有文件确实存在的前提下，才会发生扩展。如果文件不存在，扩展就不会发生。</p>
...
</div><p>上面例子中，如果 <code v-pre>?.txt</code> 可以扩展成文件名，<code v-pre>echo</code> 命令会输出扩展后的结果；如果不能扩展成文件名，<code v-pre>echo</code> 就会原样输出 <code v-pre>?.txt</code>。</p>
<h2 id="字符扩展-1" tabindex="-1"><a class="header-anchor" href="#字符扩展-1"><span><code v-pre>*</code> 字符扩展</span></a></h2>
...
</div><p>上面例子中，<code v-pre>*.txt</code> 代表后缀名为 <code v-pre>.txt</code> 的所有文件。</p>
<p>如果想输出当前目录的所有文件，直接用 <code v-pre>*</code> 即可。</p>
...
</div><p><code v-pre>*</code> 可以匹配空字符，下面是一个例子。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#A0A1A7;--shiki-light-font-style:italic;--shiki-dark:#7F848E;--shiki-dark-font-style:italic"># 存在文件 a.txt、b.txt 和 ab.txt</span></span>
...
</div><p>注意，<code v-pre>*</code> 不会匹配隐藏文件(以 <code v-pre>.</code> 开头的文件)，即 <code v-pre>ls *</code> 不会输出隐藏文件。</p>
<p>如果要匹配隐藏文件，需要写成 <code v-pre>.*</code>。</p>
...
</div><p>如果要匹配隐藏文件，同时要排除 <code v-pre>.</code> 和 <code v-pre>..</code> 这两个特殊的隐藏文件，可以与方括号扩展结合使用，写成 <code v-pre>.[!.]*</code>。</p>
...
</div><p>注意，<code v-pre>*</code> 字符扩展属于文件名扩展，只有文件确实存在的前提下才会扩展。如果文件不存在，就会原样输出。</p>
...
</div><p>上面例子中，当前目录里面没有 <code v-pre>c</code> 开头的文件，导致 <code v-pre>c*.txt</code> 会原样输出。</p>
<p><code v-pre>*</code> 只匹配当前目录，不会匹配子目录。</p>
...
</div><p>上面的例子，文本文件在子目录，<code v-pre>*.txt</code> 不会产生匹配，必须写成 <code v-pre>*/*.txt</code>。有几层子目录，就必须写几层星号。</p>
<p>Bash 4.0 引入了一个参数 <code v-pre>globstar</code>，当该参数打开时，允许 <code v-pre>**</code> 匹配零个或多个子目录。因此，<code v-pre>**/*.txt</code> 可以匹配顶层的文本文件和任意深度子目录的文本文件。详细介绍请看后面 <code v-pre>shopt</code> 命令的介绍。</p>
<h2 id="方括号扩展" tabindex="-1"><a class="header-anchor" href="#方括号扩展"><span>方括号扩展</span></a></h2>
<p>方括号扩展的形式是 <code v-pre>[...]</code>，只有文件确实存在的前提下才会扩展。如果文件不存在，就会原样输出。括号之中的任意一个字符。比如，<code v-pre>[aeiou]</code> 可以匹配五个元音字母中的任意一个。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#A0A1A7;--shiki-light-font-style:italic;--shiki-dark:#7F848E;--shiki-dark-font-style:italic"># 存在文件 a.txt 和 b.txt</span></span>
...
</div><p>上面例子中，<code v-pre>[ab]</code> 可以匹配 <code v-pre>a</code> 或 <code v-pre>b</code>，前提是确实存在相应的文件。</p>
...
</div><p>上面例子中，由于扩展后的文件不存在，<code v-pre>[ab].txt</code> 就原样输出了，导致 <code v-pre>ls</code> 命名报错。</p>
<p>方括号扩展还有两种变体: <code v-pre>[^...]</code> 和 <code v-pre>[!...]</code>。它们表示匹配不在方括号里面的字符，这两种写法是等价的。比如，<code v-pre>[^abc]</code> 或 <code v-pre>[!abc]</code> 表示匹配除了 <code v-pre>a</code>、<code v-pre>b</code>、<code v-pre>c</code> 以外的字符。</p>
...
</div><p>上面命令中，<code v-pre>[!a]</code> 表示文件名第二个字符不是 <code v-pre>a</code> 的文件名，所以返回了 <code v-pre>aba</code> 和 <code v-pre>bbb</code> 两个文件。</p>
<p>注意，如果需要匹配 <code v-pre>[</code> 字符，可以放在方括号内，比如 <code v-pre>[[aeiou]</code>。如果需要匹配连字号 <code v-pre>-</code>，只能放在方括号内部的开头或结尾，比如 <code v-pre>[-aeiou]</code> 或 <code v-pre>[aeiou-]</code>。</p>
<h2 id="start-end-扩展" tabindex="-1"><a class="header-anchor" href="#start-end-扩展"><span>[start-end] 扩展</span></a></h2>
<p>方括号扩展有一个简写形式 <code v-pre>[start-end]</code>，表示匹配一个连续的范围。比如，<code v-pre>[a-c]</code> 等同于 <code v-pre>[abc]</code>，<code v-pre>[0-9]</code> 匹配 <code v-pre>[0123456789]</code>。</p>
...
<ul>
<li><code v-pre>[a-z]</code>: 所有小写字母。</li>
<li><code v-pre>[a-zA-Z]</code>: 所有小写字母与大写字母。</li>
<li><code v-pre>[a-zA-Z0-9]</code>: 所有小写字母、大写字母与数字。</li>
<li><code v-pre>[abc]*</code>: 所有以 <code v-pre>a</code>、<code v-pre>b</code>、<code v-pre>c</code> 字符之一开头的文件名。</li>
<li><code v-pre>program.[co]</code>: 文件 <code v-pre>program.c</code> 与文件 <code v-pre>program.o</code>。</li>
<li><code v-pre>BACKUP.[0-9][0-9][0-9]</code>: 所有以 <code v-pre>BACKUP.</code> 开头，后面是三个数字的文件名。</li>
</ul>
<p>这种简写形式有一个否定形式 <code v-pre>[!start-end]</code>，表示匹配不属于这个范围的字符。比如，<code v-pre>[!a-zA-Z]</code> 表示匹配非英文字母的字符。</p>
...
</div><p>上面代码中，<code v-pre>[!1-3]</code> 表示排除 1、2 和 3。</p>
<h2 id="大括号扩展" tabindex="-1"><a class="header-anchor" href="#大括号扩展"><span>大括号扩展</span></a></h2>
...
</div><p>上面例子中，即使不存在对应的文件，<code v-pre>{a,b,c}</code> 依然扩展成三个文件名，导致 <code v-pre>ls</code> 命令报了三个错误。</p>
...
</div><p>上面例子中，会先进行大括号扩展，然后进行 <code v-pre>*</code> 扩展。</p>
...
</div><p>由于大括号扩展 <code v-pre>{...}</code> 不是文件名扩展，所以它总是会扩展的。这与方括号扩展 <code v-pre>[...]</code> 完全不同，如果匹配的文件不存在，方括号就不会扩展。这一点要注意区分。</p>
...
<span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">a.txt</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> b.txt</span></span></code></pre>
</div><p>上面例子中，如果不存在 <code v-pre>a.txt</code> 和 <code v-pre>b.txt</code>，那么 <code v-pre>[ab].txt</code> 就会变成一个普通的文件名，而 <code v-pre>{a,b}.txt</code> 可以照样扩展。</p>
<h2 id="start-end-扩展-1" tabindex="-1"><a class="header-anchor" href="#start-end-扩展-1"><span>{start..end} 扩展</span></a></h2>
<p>大括号扩展有一个简写形式 <code v-pre>{start..end}</code>，表示扩展成一个连续序列。比如，<code v-pre>{a..z}</code> 可以扩展成 26 个小写英文字母。</p>
...
<p>这个写法的另一个常见用途，是直接用于 <code v-pre>for</code> 循环。</p>
...
<p>如果整数前面有前导 <code v-pre>0</code>，扩展输出的每一项都有前导 <code v-pre>0</code>。</p>
...
</div><p>这种简写形式还可以使用第二个双点号(<code v-pre>start..end..step</code>)，用来指定扩展的步长。</p>
...
</div><p>上面代码将 <code v-pre>0</code> 扩展到 <code v-pre>8</code>，每次递增的长度为 <code v-pre>2</code>，所以一共输出 5 个数字。</p>
...
</div><h2 id="变量扩展" tabindex="-1"><a class="header-anchor" href="#变量扩展"><span>变量扩展</span></a></h2>
<p>Bash 将美元符号 <code v-pre>$</code> 开头的词元视为变量，将其扩展成变量值，详见<RouteLink to="/linux/bash/variable.html">Bash 变量</RouteLink> 一章。</p>
...
</div><p>变量名除了放在美元符号后面，也可以放在 <code v-pre>${}</code> 里面。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">$</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> echo</span><span style="--shiki-light:#E45649;--shiki-dark:#ABB2BF"> ${</span><span style="--shiki-light:#E45649;--shiki-dark:#E06C75">SHELL</span><span style="--shiki-light:#E45649;--shiki-dark:#ABB2BF">}</span></span>
<span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">/bin/bash</span></span></code></pre>
</div><p><code v-pre>${!string*}</code> 或 <code v-pre>${!string@}</code> 返回所有匹配给定字符串 <code v-pre>string</code> 的变量名。</p>
...
</div><p>上面例子中，<code v-pre>${!S*}</code> 扩展成所有以 <code v-pre>S</code> 开头的变量名。</p>
<h2 id="子命令扩展" tabindex="-1"><a class="header-anchor" href="#子命令扩展"><span>子命令扩展</span></a></h2>
...
</div><p>上面例子中，<code v-pre>$(date)</code> 返回 <code v-pre>date</code> 命令的运行结果。</p>
...
<span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">Tue</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> Jan</span><span style="--shiki-light:#986801;--shiki-dark:#D19A66"> 28</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> 00:01:13</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> CST</span><span style="--shiki-light:#986801;--shiki-dark:#D19A66"> 2020</span></span></code></pre>
</div><p><code v-pre>$(...)</code> 可以嵌套，比如 <code v-pre>$(ls $(pwd))</code>。</p>
<h2 id="算术扩展" tabindex="-1"><a class="header-anchor" href="#算术扩展"><span>算术扩展</span></a></h2>
<p><code v-pre>$((...))</code> 可以扩展成整数运算的结果，详见 <RouteLink to="/linux/bash/arithmetic.html">Bash 的算术运算</RouteLink> 一章。</p>
...
</div><h2 id="字符类" tabindex="-1"><a class="header-anchor" href="#字符类"><span>字符类</span></a></h2>
<p><code v-pre>[[:class:]]</code> 表示一个字符类，扩展成某一类特定字符之中的一个。常用的字符类如下。</p>
<ul>
<li><code v-pre>[[:alnum:]]</code>: 匹配任意英文字母与数字</li>
<li><code v-pre>[[:alpha:]]</code>: 匹配任意英文字母</li>
<li><code v-pre>[[:blank:]]</code>: 空格和 Tab 键。</li>
<li><code v-pre>[[:cntrl:]]</code>: ASCII 码 0-31 的不可打印字符。</li>
<li><code v-pre>[[:digit:]]</code>: 匹配任意数字 0-9。</li>
<li><code v-pre>[[:graph:]]</code>: A-Z、a-z、0-9 和标点符号。</li>
<li><code v-pre>[[:lower:]]</code>: 匹配任意小写字母 a-z。</li>
<li><code v-pre>[[:print:]]</code>: ASCII 码 32-127 的可打印字符。</li>
<li><code v-pre>[[:punct:]]</code>: 标点符号(除了 A-Z、a-z、0-9 的可打印字符)。</li>
<li><code v-pre>[[:space:]]</code>: 空格、Tab、LF(10)、VT(11)、FF(12)、CR(13)。</li>
<li><code v-pre>[[:upper:]]</code>: 匹配任意大写字母 A-Z。</li>
<li><code v-pre>[[:xdigit:]]</code>: 16 进制字符(A-F、a-f、0-9)。</li>
</ul>
<p>请看下面的例子。</p>
...
<p>字符类的第一个方括号后面，可以加上感叹号 <code v-pre>!</code>，表示否定。比如，<code v-pre>[![:digit:]]</code> 匹配所有非数字。</p>
...
<h2 id="使用注意点" tabindex="-1"><a class="header-anchor" href="#使用注意点"><span>使用注意点</span></a></h2>
...
<span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">ab.txt</span></span></code></pre>
</div><p>上面命令的执行过程是，Bash 先将 <code v-pre>a*.txt</code> 扩展成 <code v-pre>ab.txt</code>，然后再执行 <code v-pre>ls ab.txt</code>。</p>
...
</div><p>上面代码中，由于不存在 <code v-pre>r</code> 开头的文件名，<code v-pre>r*</code> 会原样输出。</p>
<p>下面是另一个例子。</p>
...
</div><p>另外，前面已经说过，大括号扩展 <code v-pre>{...}</code> 不是文件名扩展。</p>
<p>(3)<strong>只适用于单层路径。</strong></p>
<p>所有文件名扩展只匹配单层路径，不能跨目录匹配，即无法匹配子目录里面的文件。或者说，<code v-pre>?</code> 或 <code v-pre>*</code> 这样的通配符，不能匹配路径分隔符 (<code v-pre>/</code>)。</p>
<p>如果要匹配子目录里面的文件，可以写成下面这样。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">ls</span><span style="--shiki-light:#E45649;--shiki-dark:#E5C07B"> *</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379">/</span><span style="--shiki-light:#E45649;--shiki-dark:#E5C07B">*</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379">.txt</span></span></code></pre>
</div><p>Bash 4.0 新增了一个 <code v-pre>globstar</code> 参数，允许 <code v-pre>**</code> 匹配零个或多个子目录，详见后面 <code v-pre>shopt</code> 命令的介绍。</p>
...
</div><p>上面代码创建了一个 <code v-pre>fo*</code> 文件，这时 <code v-pre>*</code> 就是文件名的一部分。</p>
<h2 id="量词语法" tabindex="-1"><a class="header-anchor" href="#量词语法"><span>量词语法</span></a></h2>
<p>量词语法用来控制模式匹配的次数。它只有在 Bash 的 <code v-pre>extglob</code> 参数打开的情况下才能使用，不过一般是默认打开的。下面的命令可以查询。</p>
...
<ul>
<li><code v-pre>?(pattern-list)</code>: 匹配零个或一个模式。</li>
<li><code v-pre>*(pattern-list)</code>: 匹配零个或多个模式。</li>
<li><code v-pre>+(pattern-list)</code>: 匹配一个或多个模式。</li>
<li><code v-pre>@(pattern-list)</code>: 只匹配一个模式。</li>
<li><code v-pre>!(pattern-list)</code>: 匹配零个或一个以上的模式，但不匹配单独一个的模式。</li>
</ul>
...
</div><p>上面例子中，<code v-pre>?(.)</code> 匹配零个或一个点。</p>
...
</div><p>上面例子中，<code v-pre>?(def)</code> 匹配零个或一个 <code v-pre>def</code>。</p>
...
</div><p>上面例子中，<code v-pre>+(.txt|.php)</code> 匹配文件有一个 <code v-pre>.txt</code> 或 <code v-pre>.php</code> 后缀名。</p>
...
</div><p>上面例子中，<code v-pre>+(.txt)</code> 匹配文件有一个或多个 <code v-pre>.txt</code> 后缀名。</p>
...
</div><p>上面例子中，由于没有可匹配的文件，<code v-pre>abc?(def)</code> 就原样输出，导致 <code v-pre>ls</code> 命令报错。</p>
<h2 id="shopt-命令" tabindex="-1"><a class="header-anchor" href="#shopt-命令"><span>shopt 命令</span></a></h2>
<p><code v-pre>shopt</code> 命令可以调整 Bash 的行为。它有好几个参数跟通配符扩展有关。</p>
<p><code v-pre>shopt</code> 命令的使用方法如下。</p>
...
<p><code v-pre>dotglob</code> 参数可以让扩展结果包括隐藏文件(即点开头的文件)。</p>
...
</div><p>打开 <code v-pre>dotglob</code>，就会包括隐藏文件。</p>
...
<p><code v-pre>nullglob</code> 参数可以让通配符不匹配任何文件名时，返回空字符。</p>
...
</div><p>上面例子中，由于当前目录不包括 <code v-pre>b</code> 开头的文件名，导致 <code v-pre>b*</code> 不会发生文件名扩展，保持原样不变，所以 <code v-pre>rm</code> 命令报错没有 <code v-pre>b*</code> 这个文件。</p>
<p>打开 <code v-pre>nullglob</code> 参数，就可以让不匹配的通配符返回空字符串。</p>
...
</div><p>上面例子中，由于没有 <code v-pre>b*</code> 匹配的文件名，所以 <code v-pre>rm b*</code> 扩展成了 <code v-pre>rm</code>，导致报错变成了”缺少操作数“。</p>
</li>
<li>
<p><strong>failglob 参数</strong></p>
<p><code v-pre>failglob</code> 参数使得通配符不匹配任何文件名时，Bash 会直接报错，而不是让各个命令去处理。</p>
...
</div><p>上面例子中，打开 <code v-pre>failglob</code> 以后，由于 <code v-pre>b*</code> 不匹配任何文件名，Bash 直接报错了，不再让 <code v-pre>rm</code> 命令去处理。</p>
</li>
<li>
<p><strong>extglob 参数</strong></p>
<p><code v-pre>extglob</code> 参数使得 Bash 支持 ksh 的一些扩展语法。它默认应该是打开的。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">$</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> shopt</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> extglob</span></span>
<span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">extglob</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379">         on</span></span></code></pre>
...
<p><code v-pre>nocaseglob</code> 参数可以让通配符扩展不区分大小写。</p>
...
</div><p>上面例子中，打开 <code v-pre>nocaseglob</code> 以后，<code v-pre>program*</code> 就不区分大小写了，可以匹配 <code v-pre>ProgramData</code> 等。</p>
</li>
<li>
<p><strong>globstar 参数</strong></p>
<p><code v-pre>globstar</code> 参数可以使得 <code v-pre>**</code> 匹配零个或多个子目录。该参数默认是关闭的。</p>
<p>假设有下面的文件结构。</p>
<div class="language-bash" data-highlighter="shiki" data-ext="bash" data-title="bash" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">a.txt</span></span>
...
</div><p>上面的文件结构中，顶层目录、第一级子目录 <code v-pre>sub1</code>、第二级子目录 <code v-pre>sub1\sub2</code> 里面各有一个文本文件。请问怎样才能使用通配符，将它们显示出来?</p>
<p>默认情况下，只能写成下面这样。</p>
...
</div><p>这是因为 <code v-pre>*</code> 只匹配当前目录，如果要匹配子目录，只能一层层写出来。</p>
<p>打开 <code v-pre>globstar</code> 参数以后，<code v-pre>**</code> 匹配零个或多个子目录。因此，<code v-pre>**/*.txt</code> 就可以得到想要的结果。</p>
