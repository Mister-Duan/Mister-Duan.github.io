<template><div><p>每一个小程序页面也可以使用 <code v-pre>.json</code> 文件来对本页面的窗口表现进行配置。</p>
<!-- more -->
<p>页面中配置项在当前页面会覆盖 <code v-pre>app.json</code> 的 <code v-pre>window</code> 中相同的配置项。文件内容为一个 JSON 对象，有以下属性:</p>
...
| singlePage                                 | Object                           | 否       | 单页模式相关配置                                                                     | 2.12.0            |</p>
<blockquote>
<p>页面配置中只能设置 <code v-pre>app.json</code> 中 <code v-pre>window</code> 对应的配置项，以决定本页面的窗口表现，所以无需写 <code v-pre>window</code> 这个属性。</p>
</blockquote>
<h2 id="singlepage" tabindex="-1"><a class="header-anchor" href="#singlepage"><span>singlePage</span></a></h2>
<blockquote>
...
<tbody>
<tr>
<td>navigationBarFit</td>
<td>String</td>
<td>否</td>
<td>默认自动调整，若原页面是自定义导航栏，则为 <code v-pre>float</code>，否则为 <code v-pre>squeezed</code></td>
<td>导航栏与页面的相交状态，值为 <code v-pre>float</code> 时表示导航栏浮在页面上，与页面相交；值为 <code v-pre>squeezed</code> 时表示页面被导航栏挤压，与页面不相交</td>
</tr>
</tbody>
</table>
