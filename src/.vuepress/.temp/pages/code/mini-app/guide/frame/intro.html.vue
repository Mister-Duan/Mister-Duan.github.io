<template><div><p>小程序开发框架的目标是通过尽可能简单、高效的方式让开发者可以在微信中开发具有原生 APP 体验的服务。</p>
<p>整个小程序框架系统分为两部分: <RouteLink to="/code/mini-app/guide/service/"><strong>逻辑层</strong></RouteLink> (App Service) 和 <RouteLink to="/code/mini-app/guide/view/"><strong>视图层</strong></RouteLink> (../view)。小程序提供了自己的视图层描述语言 <RouteLink to="/code/mini-app/guide/view/wxml.html">WXML</RouteLink> 和 <RouteLink to="/code/mini-app/guide/view/wxss.html">WXSS</RouteLink>，以及基于 JavaScript 的逻辑层框架，并在视图层与逻辑层间提供了数据传输和事件系统，让开发者能够专注于数据与逻辑。</p>
<!-- more -->
<h2 id="响应的数据绑定" tabindex="-1"><a class="header-anchor" href="#响应的数据绑定"><span>响应的数据绑定 <Badge text="务必理解" type="error" /></span></a></h2>
<p>框架的核心是一个响应的数据绑定系统，可以让数据与视图非常简单地保持同步。当做数据修改的时候，只需要在逻辑层修改数据，视图层就会做相应的更新。</p>
<p><a href="https://developers.weixin.qq.com/s/l0gLEKmv6gZa" target="_blank" rel="noopener noreferrer">在开发者工具中预览效果</a></p>
...
<li>开发者通过框架将逻辑层数据中的 <code v-pre>name</code> 与视图层的 <code v-pre>name</code> 进行了绑定，所以在页面一打开的时候会显示 <code v-pre>Hello WeChat!</code>；</li>
<li>当点击按钮的时候，视图层会发送 <code v-pre>changeName</code> 的事件给逻辑层，逻辑层找到并执行对应的事件处理函数；</li>
<li>回调函数触发后，逻辑层执行 <code v-pre>setData</code> 的操作，将 <code v-pre>data</code> 中的 <code v-pre>name</code> 从 <code v-pre>WeChat</code> 变为 <code v-pre>MINA</code>，因为该数据和视图层已经绑定了，从而视图层会自动改变为 <code v-pre>Hello MINA!</code>。</li>
</ol>
<h2 id="页面管理" tabindex="-1"><a class="header-anchor" href="#页面管理"><span>页面管理</span></a></h2>
<p><strong>框架</strong> 管理了整个小程序的页面路由，可以做到页面间的无缝切换，并给以页面完整的 <RouteLink to="/code/mini-app/guide/service/lifetime.html">生命周期</RouteLink>。开发者需要做的只是将页面的数据、方法、生命周期函数注册到 <strong>框架</strong> 中，其他的一切复杂的操作都交由 <strong>框架</strong> 处理。</p>
<h2 id="基础组件" tabindex="-1"><a class="header-anchor" href="#基础组件"><span>基础组件</span></a></h2>
<p><strong>框架</strong> 提供了一套基础的 <RouteLink to="/code/mini-app/guide/view/component.html">组件</RouteLink>，<RouteLink to="/code/mini-app/guide/view/component.html">这些组件</RouteLink> 自带微信风格的样式以及特殊的逻辑，开发者可以通过组合基础组件，创建出强大的微信小程序 。</p>
<h2 id="丰富的-api" tabindex="-1"><a class="header-anchor" href="#丰富的-api"><span>丰富的 API</span></a></h2>
<p><strong>框架</strong> 提供丰富的 <RouteLink to="/code/mini-app/guide/service/api/">微信原生 API</RouteLink>，可以方便的调起微信提供的能力，如获取用户信息，本地存储，支付功能等。</p>
</div></template>
