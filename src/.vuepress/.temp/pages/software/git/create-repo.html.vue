<template><div><p>什么是仓库呢? 英文名<strong>repository</strong>，您可以简单理解成一个目录，这个目录里面的所有文件都可以被 Git 管理起来，每个文件的修改、删除， Git 都能跟踪，以便任何时刻都可以追踪历史，或者在将来某个时刻可以“还原”。</p>
<!-- more -->
<h2 id="初始化仓库" tabindex="-1"><a class="header-anchor" href="#初始化仓库"><span>初始化仓库</span></a></h2>
...
</div><p><code v-pre>pwd</code> 命令用于显示当前目录。</p>
...
<p>第二步，通过 <code v-pre>git init</code> 命令把这个目录变成 Git 可以管理的仓库:</p>
...
</div><p>瞬间 Git 就把仓库建好了，而且告诉您是一个空的仓库(empty Git repository)，可以发现当前目录下多了一个.git 的目录，这个目录是 Git 来跟踪管理仓库的，如果这个目录里面的文件破坏了，Git 仓库也破坏了。</p>
<p>如果您没有看到 .git 目录，那是因为这个目录默认是隐藏的，用 <code v-pre>ls -ah</code> 命令就可以看见。</p>
<p>也不一定必须在空目录下创建 Git 仓库，选择一个已经有东西的目录也是可以的。</p>
<div class="hint-container caution">
<p class="hint-container-title">警告</p>
<p>请不要用已经在使用的项目练习 Git!</p>
</div>
<h2 id="把文件添加到仓库" tabindex="-1"><a class="header-anchor" href="#把文件添加到仓库"><span>把文件添加到仓库</span></a></h2>
...
<ul>
<li>具体详情请见 <RouteLink to="/code/basic/encoding.html">文件编码</RouteLink></li>
</ul>
</div>
...
</div><p>一定要放到 <code v-pre>learngit</code> 目录下(子目录也行)，因为这是一个 Git 仓库，放到其他地方 Git 再厉害也找不到这个文件。</p>
<p>把一个文件放到 Git 仓库只需要两步:</p>
<ol>
<li>
<p>用命令 <code v-pre>git add</code> 告诉 Git，把文件添加到仓库:</p>
...
</div><p>执行上面的命令，没有任何显示，这就对了，Unix 的哲学是“没有消息就是好消息”，说明添加成功。</p>
</li>
<li>
<p>用命令 <code v-pre>git commit</code> 告诉 Git，把文件提交到仓库:</p>
...
</div><p><code v-pre>git commit</code> 命令，<code v-pre>-m</code> 后面输入的是本次提交的说明，这样您就能从历史记录里方便地找到改动记录。</p>
<p><code v-pre>git commit</code> 命令执行成功后会告诉您，<code v-pre>1 file changed</code>: 1 个文件被改动(我们新添加的 readme.txt 文件)；<code v-pre>2 insertions</code>: 插入了两行内容(readme.txt 有两行内容)。</p>
</li>
</ol>
<p>为什么 Git 添加文件需要 <code v-pre>add</code>，<code v-pre>commit</code> 一共两步呢? 因为 <code v-pre>commit</code> 可以一次提交很多文件，所以您可以多次 <code v-pre>add</code> 不同的文件，比如:</p>
...
<ul>
<li>
<p>初始化一个 Git 仓库，使用 <code v-pre>git init</code> 命令。</p>
</li>
<li>
<p>添加文件到 Git 仓库，分两步:</p>
<ol>
<li>使用命令 <code v-pre>git add &lt;file&gt;</code>，可反复多次使用，添加多个文件；</li>
<li>使用命令 <code v-pre>git commit -m &lt;message&gt;</code>，完成。</li>
</ol>
</li>
</ul>
</div></template>
